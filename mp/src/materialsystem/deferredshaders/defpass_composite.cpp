...
void InitPassComposite( const defParms_composite &info, CBaseVSShader *pShader, IMaterialVar **params )
{
    ...
    if ( PARM_DEFINED( info.iEnvmap ) ) pShader->LoadCubeMap( info.iEnvmap );

    if ( PARM_DEFINED( info.iEnvmapMask ) ) pShader->LoadTexture( info.iEnvmapMask );
    ...
    if ( PARM_DEFINED( info.iSelfIllumMask ) ) pShader->LoadTexture( info.iSelfIllumMask );
}
                        ...
                        VertexCompressionType_t vertexCompression, CDeferredPerMaterialContextData *pDeferredContext )
{
    ...
    const bool bEnvmapCorrection =
        !info.bModel && bEnvmap && info.iEnvmapParallax != -1 && !params[info.iEnvmapParallax]->MatrixIsIdentity();
    ...
    const bool bSelfIllumMask =
        bSelfIllum && !bSelfIllumMaskInEnvmapMask && !bEnvmapMask && PARM_TEX( info.iSelfIllumMask );
    ...
    AssertMsgOnce( !( bTranslucent || bAlphatest ) || !bAlbedo2, "blended albedo not supported by gbuffer pass!" );
    ...
    SHADOW_STATE
    {
        ...
        {
            if ( bAlbedo2 ) iVFmtFlags |= VERTEX_COLOR;
        }
        ...
        if ( bEnvmap )
        {
            ...
            if ( bEnvmapMask )
            {
                ...
                if ( bAlbedo2 ) pShaderShadow->EnableTexture( SHADER_SAMPLER7, true );
            }
        }
        ...
        if ( bAlbedo2 )
        {
            ...
            if ( bBlendmodulate ) pShaderShadow->EnableTexture( SHADER_SAMPLER6, true );
        }
    ...
    DYNAMIC_STATE
    {
        ...
        if ( pDeferredContext->m_bMaterialVarsChanged ||
             !pDeferredContext->HasCommands( CDeferredPerMaterialContextData::DEFSTAGE_COMPOSITE ) ||
        ...
        pShaderAPI->ExecuteCommandBuffer(
            pDeferredContext->GetCommands( CDeferredPerMaterialContextData::DEFSTAGE_COMPOSITE ) );

        if ( bGBufferNormal ) pShader->BindTexture( SHADER_SAMPLER1, GetDeferredExt()->GetTexture_Normals() );
