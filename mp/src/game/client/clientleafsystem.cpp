...
DEFINE_FIXEDSIZE_ALLOCATOR(CClientRenderablesList, 1,
...
public:
  ...
  virtual void
  ComputeTranslucentRenderLeaf(int count, const LeafIndex_t *pLeafList,
                               const LeafFogVolume_t *pLeafFogVolumeList,
                               int frameNumber, int viewID);
  virtual void
  CollateViewModelRenderables(CUtlVector<IClientRenderable *> &opaque,
...
private:
  ...
  FirstRenderableInShadow(ClientLeafShadowHandle_t shadow) {
...
private:
  ...
  CBidirectionalSet<int, ClientRenderHandle_t, unsigned int, unsigned int>
      m_RenderablesInLeaf;
...
void CClientLeafSystem::PreRender() {
  ...
  while (m_DirtyRenderables.Count()) {
    ...
    bool bThreaded =
        false; //( nDirty > 5 && cl_threaded_client_leaf_system.GetBool() &&
    ...
    if (!bThreaded) {
      ...
      ParallelProcess("CClientLeafSystem::PreRender", pDirtyRenderables, nDirty,
                      this, &CClientLeafSystem::InsertIntoTree,
                      &CClientLeafSystem::FrameLock,
...
CClientLeafSubSystemData *
CClientLeafSystem::GetSubSystemDataInLeaf(int leaf, int nSubSystemIdx) {
  assert(nSubSystemIdx < N_CLSUBSYSTEMS);
...
CClientLeafSystem::AddShadow(ClientShadowHandle_t userId,
                             unsigned short flags) {
  ...
  if (count < m_RenderablesInLeaf.NumAllocated()) {
    ...
    if (count >= 20000) {
      for (int j = 0; j < m_RenderablesInLeaf.NumAllocated(); j++) {
        ...
        C_BaseEntity *pEnt =
            info.m_pRenderable->GetIClientUnknown()->GetBaseEntity();
        if (pEnt) {
          pClassName = pEnt->GetClassname();
        } else {
          CNewParticleEffect *pEffect =
              dynamic_cast<CNewParticleEffect *>(info.m_pRenderable);
          if (pEffect) {
            Vector mins, maxs;
            pEffect->GetRenderBounds(mins, maxs);
            Q_snprintf(pTemp, sizeof(pTemp), "ps: %s %.2f,%.2f",
                       pEffect->GetEffectName(), maxs.x - mins.x,
                       maxs.y - mins.y);
            pClassName = pTemp;
          } else if (dynamic_cast<CParticleEffectBinding *>(
                         info.m_pRenderable)) {
        ...
        Msg(" %d: %p group:%d %s %d %d TransCalc:%d renderframe:%d\n", j,
            info.m_pRenderable, info.m_RenderGroup, pClassName, info.m_LeafList,
            info.m_RenderLeaf, info.m_TranslucencyCalculated,
            info.m_RenderFrame);
      }
...
//-----------------------------------------------------------------------------
inline bool
CClientLeafSystem::IsViewModelRenderGroup(RenderGroup_t group) const {
...
void CClientLeafSystem::DrawDetailObjectsInLeaf(int leaf, int nFrameNumber,
  ...
  if (!m_Leaf.IsValidIndex(leaf)) {
    AssertMsg(
        false,
        "DrawDetailObjectsInLeaf: Invalid leaf index %d (m_Leaf.Count()=%d)",
        leaf, m_Leaf.Count());
...
#define LeafToMarker(leaf)                                                     \
  reinterpret_cast<RenderableInfo_t *>(((intp)(leaf) << 1) | 1)
...
#define MarkerToLeaf(p) (int)((reinterpret_cast<size_t>(p)) >> 1)
  ...
  bool bThreaded = false; //( cl_threaded_client_leaf_system.GetBool() &&
                          // g_pThreadPool->NumThreads() );
  int globalFrameCount = gpGlobals->framecount;
  int i;

  static CUtlVector<RenderableInfo_t *>
      orderedList; // @MULTICORE (toml 8/30/2006): will need to make non-static
                   // if thread this function
  ...
  if (bThreaded) {
    ParallelProcess("CClientLeafSystem::ComputeTranslucentRenderLeaf",
                    renderablesToUpdate.Base(), renderablesToUpdate.Count(),
